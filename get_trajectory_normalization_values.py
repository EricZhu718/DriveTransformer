...
def compute_all_differentials(dataset, output_file='ego_trajectory_differentials_train.npz', save_interval=10000):
    ...
    if checkpoint_path.exists():
        ...
        original_trajectories = checkpoint['original_trajectories'].tolist()
        masks = checkpoint['masks'].tolist()
    ...
    for idx in tqdm(range(start_idx, total_samples), desc="Computing differentials"):
        try:
            ...
            original_trajectories.append(data_info['ego_fut_trajs_fix_time'])
            masks.append(data_info['ego_fut_masks_fix_time'])
            ...
            if (idx + 1) % save_interval == 0:
                ...
                np.savez_compressed(
                    ...
                    original_trajectories=np.array(original_trajectories, dtype=object),
    ...
    original_trajectories_array = np.array(original_trajectories, dtype=np.float32)  # Shape: (N, 30, 2)
    masks_array = np.array(masks, dtype=np.float32)  # Shape: (N, 30)
    ...
    np.savez_compressed(
        ...
        original_trajectories=original_trajectories_array,
        masks=masks_array,
    ...
    print(f"Original trajectories array shape: {original_trajectories_array.shape}")
    print(f"Masks array shape: {masks_array.shape}")
    return differentials_array, original_trajectories_array, masks_array, metadata_array
...
def main():
    ...
    differentials, original_trajectories, masks, metadata = compute_all_differentials(dataset, output_file=output_file, save_interval=10000)
    ...
    print(f"original_trajectories = data['original_trajectories']  # Shape: {original_trajectories.shape}")
    print(f"masks = data['masks']  # Shape: {masks.shape}")
